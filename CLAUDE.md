...
- `DotaRankBadge.tsx` (медали Рекрут…Титан, tier=rank_tier//10, звёзды %10),
  ...
  **Висяки прозвона** (17.09, «не всегда получается зайти через
  ...
  пустой call_active; хвост без оффера при flush не применяется; close/
  error СТАРОГО peer'а игнорируются, если слот уже занят новым (simple-peer
  шлёт close микротаской ПОСЛЕ map.set — раньше удалял НОВЫЙ peer и играл
  отбой), пересборка помечена `silentPeers`. **Смена микрофона** —
...
позже десктопа с меньшим id; ансвер/кандидат без peer'а — дроп. **Приём
...
ВНИМАНИЕ: PR #79 хозяин смержил и выпустил как 2.3.11 ВЕЧЕРОМ 16.09 —
только со стилем «Б» (⛽ там ещё линейный). Три мобильных пакета и
возврат ⛽ (десктоп 2.3.12) уехали отдельным PR (грабля №14). Осталось:
