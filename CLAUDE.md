...
- Пуш-данные message/call несут chat_name; для ЛС (chat.name=NULL) —
  ...
  `peer_user_id` в ЛС = ОТПРАВИТЕЛЬ/звонящий/зовущий (собеседник ДЛЯ
  ПОЛУЧАТЕЛЯ пуша; chats.py файлы, ws/handler.py сообщения и звонки,
  dota.py, polls.py). До 22.09 три из четырёх мест брали «не
  отправителя» — то есть самого получателя, и деп-линк мобилки открывал
  ЛС «с самим собой» (шапка без аватарки, звонок себе).
...
- Покер: `poker_engine.py` (колода, оценка 7→5), `poker_game.py`
  ...
  **«Встать» посреди турнира** (22.09; до этого сервер лишь ставил
  is_active=False, движок держал игрока в раздачах, клиенты рисовали его
  за столом — «встать не работает»): `poker_game.leave_game` — сдался:
  в живой раздаче фолд (свой ход → `_advance`, чужой → только пометка,
  очередь за чужой ход двигать нельзя), стек 0, `g.left` (нет докупки —
  `can_reenter`, `public_view` отдаёт `left`), энтри «за газ» остаётся в
  котле как при вылете; `leave_table` в живой раздаче зовёт
  `ws.handler.broadcast_and_continue` (общий хвост с handle_poker_action:
  состояние всем, фаст-форвард, конец раздачи → стеки/следующая/финал —
  если после фолда остался один, турнир закрывается тут же), между
  раздачами только рассылает состояние — таймер следующей раздачи сам
  увидит стек 0, а `start_hand`, объявив финал, доводит его через
  `_finish_tournament` (правка в `_next`). Ремни: `g.finalized_hand_no` —
  `_finish_hand_and_maybe_next` доводит раздачу один раз (обычный поток ×
  фаст-форвард × «встать» ставили бы второй таймер = двойная сдача),
  `g.fast_forwarding` — один `_ff` на раздачу. Клиенты (оба): место с
  `is_active=false` / игрок с `left` — пустой слот, «Встать»/панель
  действий по активному месту, счётчик игроков без вставших. Тест:
  scratchpad `test_poker_leave.py` (TestClient, выход не в свой ход /
  в свой ход при двух и при трёх).
...
снята в RN 0.82), edge-to-edge принудительно (окно под клавиатуру больше
НЕ ужимается — adjustResize при setDecorFitsSystemWindows(false)
игнорируется, а штатный `KeyboardAvoidingView` на Android БЕСПОЛЕЗЕН:
RN шлёт keyboardDidShow с `screenY` по видимой рамке окна, которая тут не
меняется, и KAV считает перекрытие нулевым — клавиатура накрывала поле
ввода целиком в 0.9.0–0.9.2 (грабля №17). Лечение —
`components/KeyboardInset.tsx` вокруг RootNavigator в App.tsx: эмуляция
adjustResize, всё приложение ужимается над клавиатурой, как на старых
сборках; высота — `useAnimatedKeyboard` из reanimated (нативный слушатель
инсета ime() на decorView, покадрово, панель функций/подсказок клавиатуры
входит — она часть окна IME) с ОБОИМИ флагами translucent (иначе
reanimated дописывает отступы системных полос корню Activity — двойной
статус-бар) минус `useSafeAreaInsets().bottom` (высота там от нижнего края
экрана; safe-area ime() не включает). Натив reanimated в сборке есть —
едет по OTA; хук deprecated в пользу react-native-keyboard-controller,
тот потребует нового APK. Нижний инсет системной полосы — у BottomTabs и
у футера MediaViewer); React 19; expo-av УДАЛЁН —
...
`addTransceiver("video", {direction:"sendrecv", streams:[localStream]})` и
кладёт его sender в `videoSenders` — как десктоп. `streams` ОБЯЗАТЕЛЕН:
он даёт m-line msid нашего потока; без него видеодорожка приезжала на
десктоп «сиротой» (`event.streams` пуст), а simple-peer на такие не
реагирует вовсе (ни 'stream', ни 'track') — включённая на телефоне
камера на компе была чёрным экраном. С msid дорожка входит в тот же
поток, что и звук, и десктоп получает её при первом согласовании;
msid, появившийся позже (ренегосиацией), браузер в новый 'track' не
превращает — поэтому именно при создании транссивера. Без неё в согласованном SDP видео нет
...
кнопка «Сохранить» — В ФУТЕРЕ под контентом, а не оверлеем: абсолютная
накрывала дорожку перемотки нативных контролов).
...
Параметры из пуша неполные (только id и имя чата), а `peer_user_id` у
серверов до 22.09 был СВОИМ id получателя (сервер брал «не отправителя»
относительно отправителя — см. раздел сервера): шапка была с пустым
кружком / своей аватаркой, тап по ней открывал свой профиль, звонок из
шапки шёл бы себе. ChatScreen лечит сам: если нет `avatarUrl`, собеседник
ЛС отсутствует или равен себе, у группы нет `allowAllWrite`/`createdBy` —
берёт карточку чата из `chatApi.list()` и дописывает параметры через
`navigation.setParams` (собеседник = участник ≠ я), дальше экран как
открытый из списка. Работает и со старым сервером.
...
- **Сквозные звонки в браузере** (22.09; звонки PWA ↔ десктоп с камерой
  ...
  `shot_poker_leave.js` / `desk_poker_leave.js` — «встать» посреди турнира
  на PWA и десктопе (стол на троих через API, раздача идёт): место
  вставшего пропадает, кнопки нет, в API is_active=false.
...
17. **KeyboardAvoidingView на Android в edge-to-edge — пустышка.** RN
    0.86 считает keyboardDidShow по `getWindowVisibleDisplayFrame`, а с
    setDecorFitsSystemWindows(false) рамка не меняется: `screenY` = низ
    экрана, перекрытие «ноль», отступа нет — поле ввода накрыто целиком,
    и никакой `behavior`/`keyboardVerticalOffset` не поможет (0.9.2
    ушла хозяину с этим). Клавиатуру объезжаем корнем приложения по
    инсету ime() (`components/KeyboardInset.tsx`, reanimated); чинить
    KAV в экранах — не надо.
...
пакета нет — править оба), тумблер «Шансы» в secureStorage. Стол на
телефоне (`PokerTable`, 0.9.2): места — коробки SEAT_W×SEAT_H на овале,
радиус считается от РЕАЛЬНОГО размера стола (onLayout), чтобы коробка
целиком была внутри (раньше своё место с картами свисало за нижний край,
боковые заезжали за рамку); содержимое прижато к дальнему от центра
краю; сам стол капается и по ВЫСОТЕ видимой области (`maxHeight` из
onLayout ScrollView) — иначе своё место уезжало под панель действий и
между картами и кнопками приходилось скроллить. Проверка — Playwright на
400×800 (`scratchpad/pw/shot_poker_phone.js`: стол через API, замер
bounding-box'ов мест относительно стола).
