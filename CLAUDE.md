...
- Покер: `poker_engine.py` (колода, оценка 7→5), `poker_game.py`
  ...
  **in-memory**, рестарт сервера убивает раздачу), `api/poker.py` (столы в
  ...
  `populate_existing=True`, см. грабли №1). Джоба `close_stale_tables`
  (раз в 30 мин) сносит столы старше 6ч — лобби и игры, finished не
  трогает — с `poker_table_removed` в чат (оба клиента его обрабатывают);
  забытые столы висели в чатах днями.
...
Пакет от 15.09 (ответы хозяина зафиксированы):
1. **Покер** — создатель ДО старта настраивает стек/стартовые блайнды/
   интервал роста (+пресеты); кнопки 10/15/25/50/75% от банка у рейза;
   после победы — «сыграть ещё» не выходя со стола; режим стола «обычный»
   / «за газ ⛽»: все платят энтри, вылетевшие докупаются (число ре-энтри
   и окно настраиваются при создании; дефолты — до 3-го повышения
   блайндов, 2 ре-энтри), победитель забирает весь газ. История раздач в
   памяти пока стол жив (улицы и ставки, доска, банк, победитель, карты на
   вскрытии; карты сбросивших — никогда), кнопка «История», по умолчанию
   скрыта. Десктоп в приоритете, мобилка тоже.
2. **Приз чемпиону сезона** — пул призов (название/подсказки/вес) с
   админ-редактором в клиенте; розыгрыш запускает ТОЛЬКО админ кнопкой;
   выбор детерминирован сидом сезона; в шапке Гандолиума «🎁 ???» с
   подсказками по неделям; финальная карточка раскрывает приз. Приложение
   только выбирает и объявляет, дарит хозяин руками. Успеть до 1 октября.
3. **Единый стиль десктопа** (иконки/эмодзи/шрифт) — сначала наглядная
   страничка с 2–3 вариантами, хозяин выбирает, потом код.
...
5. Хвосты звонков: ретраи сигналинга/ICE-restart на мобилке, приём шеринга
   экрана на телефоне.
6. Покер: шпаргалка комбинаций + шансы банка на мобилку (старый висяк).
