...
- `app/ws/handler.py` — вход: ping, message (маркер `/quest_card` дропается —
  ...
  лечат mesh), плюс ПОСЛЕ каждого call_end/дисконнекта (пустой список =
  звонок кончился) и СНИМКОМ каждому новому WS-подключению (иначе
  открывший приложение посреди разговора не узнал бы о нём). `call_join
  {chat_id}` — вход в УЖЕ идущий звонок (плашка «в созвоне»): сервер
  только регистрирует участника и рассылает call_active, mesh дособирают
  клиенты tie-break'ом (меньший id офферит) — глейра нет; join в мёртвый
  звонок тихо игнорируется. Клиенты звонят ТОЛЬКО на signal.type=offer
  (кандидаты/ансверы/ре-офферы камеры звонок не поднимают), входящий
  живёт 20с и гаснет ЛОКАЛЬНО (не decline). Исходящие: message,
...
offer + takenRef по call_taken; входящий гаснет сам через 20с (локально);
рингтон мягкий «как на компе» — сигнал/5с + один импульс вибрации,
volume 0.6; activeCalls (chat_id→[user_id]) в CallContext по call_active,
в ChatScreen плашка «В созвоне: имена» (тап/кнопка звонка = joinOngoing →
call_join, свой звонок = развернуть). Кнопка звонка в шапке умная: созвон
идёт → войти, нет → дозвон (ЛС).
