...
снята в RN 0.82), edge-to-edge принудительно (на Android 15 окно под
клавиатуру больше НЕ ужимается — `KeyboardAvoidingView` в ChatScreen с
`behavior="padding"` на ОБЕИХ платформах, иначе клавиатура накрывала поле
ввода целиком; на старых Android окно ещё ужимается само, а KAV считает
перекрытие по реальным рамкам и лишнего не добавляет; нижний инсет
системной полосы — у BottomTabs и у футера MediaViewer); React 19; expo-av УДАЛЁН —
...
В payload пуша нет аватарки — ChatScreen добирает её сам
(`headerAvatar`: ЛС — `userApi.getUser`, группа — карточка из
`chatApi.list`), иначе шапка из пуша была с пустым кружком.
