...
**Фоновый сервис звонка** (`callForegroundService.ts`): с Android 14
уведомление ОБЯЗАНО нести `foregroundServiceTypes` — MICROPHONE всегда,
CAMERA только когда камера реально включена (тип требует выданного
разрешения, в голосовом звонке его может не быть). Без типа camera
система ОТБИРАЕТ камеру у свёрнутого приложения: дорожка остаётся, кадры
нет, у собеседника вечный стоп-кадр даже после возврата (лечили ручным
«выкл/вкл») — плюс «Гандола не отвечает» у того, у кого камера включена.
...
1. Адаптивность PWA на широких экранах (два столбца, как договорились).
2. Хвосты звонков: ретраи сигналинга/ICE-restart на мобилке, приём шеринга
   экрана на телефоне; самсунг ANR/нагрев — наблюдать после камеры-офф.
3. Покер: шпаргалка комбинаций + шансы банка на мобилку (старый висяк).

СДЕЛАНО, ждёт релиза: мобилка 0.8 (динамик + персональная громкость) —
version 0.8.0/versionCode 8 уже подняты, APK соберётся Actions'ом при
мерже в main, ссылка `mobile-latest` и QR в профиле подхватят сами.
