...
- `app/backups.py` — ночные дампы БД: 04:00 МСК `pg_dump -Fc` в
  ...
  Дампы — ТОЛЬКО БД: uploads (фото/голосовые/видео) в бэкап не входят.
...
- `pages/Main.tsx` — тайтлбар с меню режимов **chat / compendium**
  ...
  этой сессии; после перезапуска — везде переписка.
...
грабля №15). Ветка после этого перезапущена от main. Осталось:
...
3. Покер: **стол рядом с перепиской** (вариант 2 от 17.09) — на экране
   стола показывать ленту этого чата узкой колонкой справа (вместо
   MemberList), чтобы переговариваться во время раздачи. Вариант 1
   (покер как часть чата, не режим) сделан в 2.3.13.
