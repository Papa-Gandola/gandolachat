...
- `prizes.py` — **приз чемпиону сезона** (миграция 0014: season_prizes —
  ...
  ошибка приза роллбэчится и финал не роняет. Финал закрывает ПРОШЛЫЙ
...
финале). После мержа хозяину: сервер по правилам (миграции 0013/0014
...
Осталось:
1. **Единый стиль десктопа** (иконки/эмодзи/шрифт) — наглядная страничка
   с вариантами А/Б/В отдана, хозяин выбирает, потом код.
2. Адаптивность PWA на широких экранах (два столбца, как договорились).
3. Хвосты звонков: ретраи сигналинга/ICE-restart на мобилке, приём шеринга
   экрана на телефоне.
4. Покер: шпаргалка комбинаций + шансы банка на мобилку (старый висяк).
