...
- `app/api/polls.py` — **опросы + закрепы** (миграция 0012: polls,
  ...
  WS `poll_updated` шлёт ПОЛНЫЙ PollOut с mine=false — клиенты мержат
  mine из своего прежнего стейта (свои действия приходят HTTP-ответом).
...
1. Релиз мобилки 0.8 (новый APK, versionCode 8): динамик/громкая связь +
