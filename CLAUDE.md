...
Discord-подобный мессенджер для чата друзей: ~50 зарегистрировано, реально
активных — 9–10 человек (слова хозяина, 17.09) — лимиты OpenDota/пушей/
покера считать от этой цифры. Личный проект Гандолы (Papa-Gandola). Общение — по-русски; хозяин на Windows 11, знает Java/Python.
...
- **Единый стиль «Б»** (2.3.11, выбор хозяина): (А) кнопки и служебные
  ...
  Twemoji, НЕ линейная иконка: линейную колонку хозяин попросил вернуть
  назад на второй день, «⛽ нравился всем» — не менять);
...
`recover()` зовётся и на AppState active (CallContext). **Приём экрана с
десктопа**: десктоп шарит ОТДЕЛЬНЫМ simple-peer'ом на каждого
...
компа, экран опоздавшему) — сделаны 16.09 в PR #79 (OTA, натив не
менялся). Экран С телефона — отдельная нативная задача (новый APK).
Покерная шпаргалка + «Шансы» на мобилке — сделаны 16.09 (PR #79):
...
Два столбца PWA на широких экранах — сделаны 17.09 (PR #79, см. раздел
мобилки). Осталось:
