...
- `finale.py` — **финал сезона**: cron 1-го числа 04:10 МСК (01:10 UTC,
  misfire 20ч), под общим `_JOB_LOCK` поллера. Закрывает ВСЕ незакрытые
  прошлые сезоны (не только вчерашний — даунтайм дольше grace не оставит
  месяц без финала): снапшот таблицы в season_results (идемпотентно —
  сезоны, уже имеющие строки, пропускаются; только gas>0; сортировка
  -gas, тай-брейк username), карточка `/quest_card kind=season_final`
  (season_name родительным — MONTHS_GEN, podium топ-3) от ИМЕНИ ЧЕМПИОНА
  во все compendium_enabled-группы + пуш. Цикл по чатам — снапшот id +
  перечитка Chat/User на итерацию, ошибка одного чата роллбэчится и не
  травит остальные (грабля №2). Награды НЕ в колонках — выводятся из
  season_results запросами. Поздний парс (recheck 36ч) может докапать газ
  в закрытый сезон — снапшот НЕ пересчитывается, это осознанно.
